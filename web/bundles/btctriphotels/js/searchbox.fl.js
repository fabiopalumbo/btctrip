...
Nibbler.Searchbox.js.Searchbox.Box.Hotels = function (d) {
    ...
    this.init = function () {
        ...
        this.addModules([{
            ...
            name: "multiple",
            module: k.Hotels.MultipleDestinations,
            selector: "div.ctn-multipledestination"
        }, {
            name: "passengers",
            module: k.Hotels.FlightPassengersDropdown,
            selector: "div.mod-passengers-flights",
            store: true
        }, {
            name: "advanced",
            module: k.Hotels.AdvancedSearch,
    ...
    this.setupTripTypes = function () {
        ...
        j.options.context.find("div.com-adddestiny").on("click", function () {
            ...
            if (k.originText) {
                j.observer.notify("hotels.multiple.0.origin.update", {
                    place: k.originText,
                    code: k.originValue
                })
            }
            if (k.destinationText) {
                j.observer.notify("hotels.multiple.0.destination.update", {
...
Nibbler.Searchbox.js.Searchbox.Searcher.Hotels = function (c) {
    this.options = {
        urlSearchPrefix: urlSearchPrefixG,
    ...
    this.getFormattedAdvancedOptions = function (g) {
        ...
        if (h) {
            if (b[a] == f) {
                return [g.departureTime, g.classHotel, g.scaleHotel, (g.airlineHotel != "") ? g.airlineHotel : "NA"]
            } else {
                return [g.departureTime, g.returnTime, g.classFlight, g.scaleFlight, (g.airlineHotel != "") ? g.airlineHotel : "NA"]
            }
        } else {
            return []
        }
...
registerNameSpace("Nibbler.Searchbox.js.Searchbox.Module.Hotels");
Nibbler.Searchbox.js.Searchbox.Module.Hotels.TripTypes = function (a) {
    var c = $.extend({
...
registerNameSpace("Nibbler.Searchbox.js.Searchbox.Module.Hotels");
Nibbler.Searchbox.js.Searchbox.Module.Hotels.MultipleDestinations = function (a) {
    this.options = {
        currentDate: new Date,
        availableDays: null,
        maxDays: null,
    ...
    this.setup = function () {
        ...
        this.options.context.find("div[class^=segment-]").each(function (g) {
            ...
            if (g > 1 && d.options.segments[g]) {
                ...
                d.addSegment({
                    currentTarget: d.options.context.find("div.com-addsegmentlink-hotels")
                })
            }
        });
        this.options.context.find("div.com-addsegmentlink-hotels").on("click", jQuery.proxy(this.addSegment, this));
    ...
    this.deleteSegment = function (c) {
        ...
        if (this.options.context.find("div[class^=segment].hidden").length > -1) {
            b.options.context.find("div.mod-addsegment-hotels").removeClass("hidden")
        }
        if (this.options.context.find("div[class^=segment].hidden").length == 4) {
            var d = b.getSegmentData(b.com[0]);
            if (d.originText) {
                b.observer.notify("hotels.places.origin.update", {
                    place: d.originText,
                    code: d.originValue
                })
            }
            if (d.destinationText) {
                b.observer.notify("hotels.places.destination.update", {
...
registerNameSpace("Nibbler.Searchbox.js.Searchbox.Module.Hotels");
Nibbler.Searchbox.js.Searchbox.Module.Hotels.HotelPassengersDropdown = function (y) {
    var u = ".com-adults-hotels select";
    var x = ".com-childrens-hotels select";
    //var H = ".com-age-hotels select";
    var t = ".com-infants-hotels select";
    ...
    function s() {
        ...
        if (g.childs) {
            a.find("option[value=" + g.childs + "]").prop("selected", true);
            a.change()
        }
      /*  var J = 0;
        if (g.infants) {
        ...
        if (g.infants) {
            c.find("option[value=" + g.childs + "]").prop("selected", true);
        }
...
Nibbler.Searchbox.js.Searchbox.Module.Hotels.AdvancedSearch = function (s) {
    var q = "div.mod-advancedsearch-hotels";
    var B = "div.com-business-hotels input[type=checkbox]";
    var t = "div.com-advancedlink";
    var y = "div.ctn-departureTime select";
    var k = "div.ctn-returnTime select";
    var F = "div.ctn-returnTime";
    var w = "div.com-scale-hotels select";
    var z = "div.com-class-hotels select";
    var d = "NA";
    var j = "C";
    ...
    function n() {
        ...
        return {
            ...
            scaleHotels: I.find(w).is(":visible") ? I.find(w).val() : f,
            classHotels: ((I.find(z).is(":visible")) || (I.find(B).val() == "C")) ? I.find(z).val() : f,
            airlineHotels: I.find(p).is(":visible") ? I.find(p).data("dataValue") : f
        }
    }
    function u(I, J) {
        if (g.id == J) {
