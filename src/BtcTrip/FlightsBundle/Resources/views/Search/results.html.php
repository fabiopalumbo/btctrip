...
Parameters may refer to values in the URL, entity properies posted as json, or both.
    ...
    from – 3 character airport or city id representing the starting point.
    to – 3 character airport or city id representing the destination.
    departureDate – The desired departure date. (format yyyy-MM-dd)
    returningDate – The desired returning date. (format yyyy-MM-dd)
    adults – Number of adults that will travel.
    children – Number of children that will travel.
    infants – Number of infants that will travel (only lap children up to 24 months).
*/
...
<div class="span3 searchbg" style="padding-top: 180px">
...
<div class="ctn-scaleandclass">
<div class="com-scale-flights">
<label>Stops</label>
<select id="slt-scale-flights">
<option value="NA">No preference</option>
<option value="NONE">Direct flights</option>
<option value="ONE">Up to 1 stop</option>
<option value="MORE_THAN_ONE">More than 1 stop</option>
</select>
</div>
...
<li class="item stops-{{data.value.code}}">
<label for="stops-{{data.value.code}}" class="{{#if data.count}} enabled {{else}} disabled {{/if}}">
<input id="stops-{{data.value.code}}" type="checkbox" name="stops" value="{{data.value.code}}" {{#if data.count}} enabled {{else}} disabled {{/if}}>
<span class="custom-label">{{filterName "stops" data.value.code}}</span>
...
<li class="item time-outbound-{{data.value.code}}">
<label for="time-outbound-{{data.value.code}}" class="{{#if data.count}} enabled {{else}} disabled {{/if}}">
<input id="time-outbound-{{data.value.code}}" type="checkbox" name="time-outbound" value="{{data.value.code}}" {{#if data.count}} enabled {{else}} disabled {{/if}}>
<span class="custom-label">{{filterName "time" data.value.code}}</span>
<span class="total">{{data.count}}</span>
</label>
</li>
{{/_each}}
...
<li class="item time-inbound-{{data.value.code}}">
<label for="time-inbound-{{data.value.code}}" class="{{#if data.count}} enabled {{else}} disabled {{/if}}">
<input id="time-inbound-{{data.value.code}}" type="checkbox" name="time-inbound" value="{{data.value.code}}" {{#if data.count}} enabled {{else}} disabled {{/if}}>
<span class="custom-label">{{filterName "time" data.value.code}}</span>
<span class="total">{{data.count}}</span>
</label>
</li>
...
<div class="span9 omega container-white" style="margin-top: 180px">
	...
	<div id="results-loader" class="results-loader">
		...
		<div class="span8 searchimagebg loader">
			...
			<div class="span8 airlinesearch">
				<div class="searchprogress iterated-text" style="padding-top: 40px">
...
<span class="commonSprite warningSymbol"></span>
<div class="text">
<span class="message">We are currently updating our website</span>
...
-->
We apologize of the inconvenience. Our new web site and services will be up and running soon.<br />
Don't hesitate to send us an email at <a href="mailto:support@btctrip.com">support@btctrip.com</a>
</span>
</div>
</div>
...
<span title="Estadísticas de puntualidad del último mes informado" class="main-sprite "></span>
{{/if}}
</li>
<li class="arrive">
<span>Arrive:</span>
...
<span class="main-sprite icon-bus" title="Airport change"></span>
{{/if}} 
</li>
<li class="airlines">
{{#_each data.airlines}}
...
<span class="price-currency {{total.perPassenger.formatted.code}}">
<span class="currency">{{total.perPassenger.formatted.mask}}</span>
<span class="amount">{{total.perPassenger.formatted.amount}}</span>
</span>
{{/each}}
<span class="fare-description price-per-passenger">Price per passenger</span>
<span class="fare-description price-detail-link "><a>Price detail</a></span>
</span>
...
<script id="popup-price-details-template" type="text/x-handlebars-template">

<ul>
<li class="item-data">
<span class="item-detail"><?php echo $sAdults . " Adult" . (($sAdults > 1) ? "s" : "") ?></span>
<span class="item-price">
{{#each this.itinerariesBox.itinerariesBoxPriceInfoList}}
<span class="price-currency {{adult.total.formatted.code}}">
<span class="currency">{{adult.total.formatted.mask}}</span>
...
<li class="item-data">
<span class="item-detail"><?php echo $sChildrensCount . " Child" . (($sChildrensCount > 1) ? "ren" : "") ?></span>
<span class="item-price">
{{#each this.itinerariesBox.itinerariesBoxPriceInfoList}}
...
<li class="perpassenger-price">
<span class="item-detail">Price per passenger</span>
<span class="item-price">
{{#each this.itinerariesBox.itinerariesBoxPriceInfoList}}
<span class="price-currency {{total.perPassenger.formatted.code}}">
<span class="currency">{{total.perPassenger.formatted.mask}}</span>
...
<span class="prices-comment">All prices includes taxes and charges.</span>
</script>
...
<a data-page-number="{{calcule currentPage 1 symbol="+"}}">Siguiente »</a>
</li>
{{else}}
...
checkoutHandlerUrl : '<?php echo $enviromentPrefix ?>/checkout/form/{hash}/{version}/{itineraryId}/{clusterIndexTraking}/INTERNATIONAL'
}
};
});
